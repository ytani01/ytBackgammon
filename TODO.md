...
**残っている項目: TODO-031, TODO-032, TODO-033 の 3 件。** これまでに 32 件を決着させた。
...
1. **TODO-032**（history フラグの付け方を見直す） — 依存が無く、盤面が
   変わらない手が履歴に挟まる実際のバグなので先に直す
2. **TODO-033**（README.md とドキュメント類を整備） — 依存が無く、
   いつでも着手できる
3. **TODO-031**（旧形式の読み込みを消す） — **まだ着手できない。**
...
- **履歴に積むかどうかは、サーバ側の `type` ごとの表で決める。**
  TODO-026 のディスパッチ表に乗せる。クライアントの
  `emit_msg()` の引数では決めない（付け忘れ・付けすぎが効かなくなる）
- **盤面が変わらないエントリは、積む前に落とす。**
  1 つ前と同じなら積まない判定を入れる
