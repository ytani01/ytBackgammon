...
**残っている項目: TODO-031, TODO-032, TODO-033, TODO-035 の 4 件。** これまでに 31 件を決着させた。
...
- [ ] `LICENSE` を置く（MIT の原文。`Copyright (c) 2020 Yoichi Tanibayashi`）
- [ ] コピーライト表記を `# (c) 2020 Yoichi Tanibayashi` に統一する
      （`src/ytbg/` の 11 ファイルと `tests/` の 11 ファイル）
- [ ] `__author__` / `__date__` を消す（`src/ytbg/` の 11 ファイル。
      `__init__.py` は `__all__` からも外す）
- [ ] `uv run pytest` / `ruff check .` / `mypy src` / `basedpyright` を通す

### きっかけ

`pyproject.toml` に `license = "MIT"` と書いてあるのに `LICENSE` が無い。
MIT はライセンス全文の同梱を条件にしているので、これだけは実害がある。

同じくファイルの先頭を見ていて、次の 2 つが出てきた。

- `__author__` / `__date__` はどこからも読まれていない。参照は
  `src/ytbg/__init__.py` の `__all__` に `'__author__'` が載っている 1 箇所だけ。
  著者は `pyproject.toml` の `authors`、日付は git が持っているので二重管理になる。
  実際にメンテされておらず、`server.py` と `__main__.py` は TODO-025 で
  中身を書き直したあとも `__date__ = '2020/05'` のままだった
- コピーライトの年が揃っていない。`src/ytbg/mylog.py` だけ `(c) 2026` で、
  残りの 22 ファイルは年が無い

### 年を 2020 にした理由

著作権表示の 3 要素として定められている「年」は、**最初の発行の年**
（17 U.S.C. § 401(b)(2)、万国著作権条約 3 条 1 項）。このリポジトリの
最初のコミットは 2020-04。

改訂を重ねるものは実質的な変更をした年を足していく実務もある
（GNU の maintainer 向け文書）が、1989-03-01 以降は米国でも表示そのものが
任意になっており、日本は無方式主義で表示に法的効果が無い。毎年の更新を
追うほどの意味は無いので、初版の年だけにする。
