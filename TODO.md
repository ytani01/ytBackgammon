...
**残っている項目: TODO-069。** これまでに 68 件を決着させた。
...
- [ ] `prefix` のあるボードのリンクと iframe の URL を、一覧ページと同じホストのパス `{prefix}/` にする（`lobby.js` の `board_url()`）
- [ ] lobby が `{prefix}/`（その下のパスも）を受けたら、`http(s)://<リクエストのホスト名>:<ボードのポート>{prefix}/…` へ 302 を返す。lobby 自身の `--prefix` の外のパスでも受ける。クエリ（`?sound=off` など）は残す
- [ ] `prefix` の無いボードは、今までどおり `http://<ホスト>:<ボードのポート>/` へのリンクにする
- [ ] 設定の `url` を無くす。書いてあれば、知らないキーとしてエラーにする（`load_config()`）
- [ ] テスト: `tests/test_lobby.py`（リダイレクト先・クエリ・lobby の prefix の外・`url` がエラーになる）、`tests/browser/lobby.test.mjs`（リンクと iframe の URL、iframe にボードが出る）
- [ ] 文書: `docs/Admin.md`（`url` の説明と「`url` は省かないこと」を消し、設定例から `url` を消す。直接開いても nginx 越しでも同じ設定で動くことを書く）、`ytbg.toml` のコメント、`CLAUDE.md` の `lobby.test.mjs` の説明
...
- `~/ytbg/ytbg.toml`（Admin.md の設定例と同じ、`url = "/ytbg1/"`）で
  ...
  `https://www.example.net:5001/ytbg1/` になって動かない。
  **直接開いて試した設定のまま、nginx を通せる形にしたい**
- 決めたこと（2026-09-16、利用者と相談）:
...
- nginx に `/ytbg1/` の `location` を書き忘れると、lobby が外から届かない
  `rpi5-1:5001` へ 302 を返す。これは書き忘れに気づく手がかりとして Admin.md に書く
