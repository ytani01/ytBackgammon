...
**残っている項目: TODO-071。** これまでに 70 件を決着させた。
...
- [ ] 大きいボードの倍率を、ウィンドウの幅と高さの両方に収まる最大の値にする
- [ ] ウィンドウの大きさを変えたら、倍率を計算し直す
- [ ] 見出しとボタンの行を含めて、大きいボードのカードが縦にも横にもはみ出ないことを確かめる
- [ ] 大きく出すボードを切り替えても、小さいボードの大きさと並びが変わらないことを確かめる
- [ ] `node --test tests/browser/lobby.test.mjs` が通る

大きいボードの中身は 1030×650px の iframe を `transform: scale(0.835)` で
縮小し、枠（`.main .frame`）を 880×560px に固定している。ウィンドウが
狭いと横にはみ出し、広いと余白が残る。

利用者と決めたこと: 幅と高さの**両方**に収まる大きさにする（縦にも
スクロールせずに、大きいボード全体が見える）。

倍率はウィンドウの大きさから `lobby.js` で計算し、CSS 変数で枠の大きさと
`scale()` に渡すつもり。iframe の大きさ（1030×650px）は変えない
（iframe を作り直すと読み込み直しになる）。

変更は小さいので実装は main が行う。倍率を計算する処理が入り挙動が
変わるので、確認の verifier に加えて reviewer も付ける。
