...
**残っている項目: TODO-022〜030 の 9 件。** これまでに 21 件を決着させた。
...
- [ ] 盤面のファイル保存・読み込みを消す（`ytbg.js` の `gen_gameinfo()` /
      `write_gameinfo()` / `read_gameinfo()` と、`index.html` で
      コメントアウトされているメニュー）
- [ ] `ytbg.js:2967` の `window.open("http://www.ytani.net:8080/...")` を消す
- [ ] `uv run pytest` / `uv run ruff check .` / `uv run mypy src` /
      `node --test tests/browser/` が通ることを確かめる

### きっかけ

TODO-020 の実装項目の 1 つ目。**早いうちにやる**（消すものを後の項目が
運ばずに済む）。場所と未使用であることは TODO-020 の調査で確かめてある。

`gen_gameinfo()` は古い `point` 形式を返し、`board.player_name`（実際は
`playername`）を読む壊れた状態。メニューは `index.html` の 63 行目から
コメントアウトされている。履歴はサーバ側に保存されているので、
盤面が失われることはない（TODO-020 で復活させないと決めた）。

### 分担

- **main** — 消すだけ。実装の担当を分けるほどの規模ではない
- **verifier** — 4 つのテストと、ブラウザでメニューの動作に影響が無いこと
- **reviewer は入れない。** 分岐や条件式が変わらない
