...
- [ ] `lobby.html` の `<h1>ytBackgammon</h1>` を消す
- [ ] `lobby.js` の `fit_main()` のコメントから h1 の話を消す
- [ ] `tests/browser/lobby.test.mjs` が通る

`fit_main()` はカードの上端の位置から空いている高さを出すので、h1 を消せば
JS の処理を変えなくても大きいボードが広がる。大きさは TODO-071 のテスト
（カードの下端がウィンドウの下端から 16px 以内）で確かめられるので、テストは足さない。
表示だけの変更で分岐は変わらないので、レビューの担当は入れない。
