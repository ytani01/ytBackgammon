...
**残っている項目: TODO-070。** これまでに 69 件を決着させた。
...
- [ ] `lobby.html` の `body` の幅の固定（`width: 920px`）を外す
- [ ] 小さいボードがウィンドウの幅に応じて横に並び、折り返すことを確かめる
- [ ] 大きいボードが先頭の 1 行を占めたままであることを確かめる
- [ ] `node --test tests/browser/lobby.test.mjs` が通る

`#boards` はすでに `display: flex; flex-wrap: wrap` だが、`body` の幅が
`920px` に固定されている。小さいボード 1 枚は枠を含めて 458px で、2 枚と
隙間で 924px になり収まらない。そのため、ウィンドウを広げても小さいボードは
1 列に縦に並ぶ。

CSS の 1 行の変更なので実装は main が行い、確認（ブラウザのテストと、
ウィンドウの幅を変えたときの見た目）を verifier に分ける。条件分岐は
変わらないのでレビューの担当は入れない。
