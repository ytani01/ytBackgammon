...
### 決まったこと（2026-09-13 に相談した）

| 論点 | 決まったこと |
|------|-------------|
| 送る形（A） | 1 つの操作を、`move` や `end_turn` のような**操作の名前で 1 通**だけ送る。クロックの切り替えはサーバの中で行う（クライアントが数えた残り時間でサーバの値を上書きしない） |
| 勝ったときのクロック（B） | **ターンが -1 になったら、サーバが両方のクロックを止める。** 勝敗の判定はクライアントのまま。`apply()` は何も送らない |
| ゲームの進行（C） | **`actions.js` に集める。** `emit_msg` を使うのはこのモジュールだけにする |
| 使用済みのダイス（D） | **今の 11〜16 のまま** `gameinfo` に入れる（保存ファイルの形は変えない）。移動を送る 1 通に含める |
| 履歴に積むか（F） | **サーバが type ごとの表で決める。** メッセージの `history` はなくし、`NO_HISTORY_TYPES` もその表にまとめる |
| 今の type | free move での移動やダイスの目の変更、名前・得点の直接の編集に使うものだけ残す。どこからも送らなくなったものは消す |
| 部品と要素（G） | **`dom.js` が作った要素を、id ではなく要素そのものとして部品に渡す。** 画像の読み込みを待つ順番（TODO-029）は変えない |
| 実装の順番 | メッセージの送り方 → JS のクラス構成 → 部品と DOM の結びつき → 細かい修正 |

### 決めること（着手してから相談する）

- **どの操作に名前を付けるか**（移動、手番を渡す、オープニングロール、
  ダブル、テイク、パス、投了など）と、**それぞれの 1 通に何を載せるか**
- 残す type と消す type の一覧
- 実装の項目（TODO-050 以降）の分け方
