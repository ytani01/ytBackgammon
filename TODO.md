...
- [ ] 大きく出すボードの iframe だけ `?sound=off` を付けずに読み込む（`lobby.js`）
- [ ] 大きく出すボードを切り替えたら、切り替わった 2 面を読み込み直す
- [ ] iframe に `allow="autoplay"` を付ける
- [ ] `tests/browser/lobby.test.mjs` で、iframe の `src` に `sound=off` が付くのが大きいボード以外だけかを見る（切り替えたあとも）
- [ ] `docs/Developer.md` の lobby の説明を直す

今は全部のボードに `?sound=off` を付けていて、lobby では音が出ない（TODO-063）。
切り替え方は、利用者と相談して「iframe を読み込み直す」に決めた
（postMessage でボードへ伝える案は、ボード側にも受け口が要るので採らなかった）。
iframe は別のオリジン（ボード自身のポート）なので、`allow="autoplay"` が無いと音が出ない。
