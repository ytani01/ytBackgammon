...
**残っている項目: TODO-068。** これまでに 67 件を決着させた。
...
対象は `README.md`・`docs/Player.md`・`docs/Admin.md`・`docs/Developer.md` だけ。
コードブロックの中は変えない。

- [ ] 見出しに、`#` の数と同じ数の `=` を付ける（`## == 見出し`）
- [ ] 文書の中に見出しへのアンカーリンクが無いことを確かめる
- [ ] Free モードの呼び方を「Freeモード」に揃える（「free move」「free move モード」
      「ルールを無視して、自由に動かせるモード」など）。**ヘッダのチェックボックスの
      名前として書いている「Free」はそのまま残す**
- [ ] 丸括弧を半角 `()` に揃える（全角 `（）` を直す）

見出しは、階層が分かりにくいため（利用者の依頼）。4 文書で 91 個（h1〜h4）。
GitHub のアンカーは見出しの文字から作られるので、外から見出しへ張ったリンクは切れる。
立てた時点では、文書の中にアンカーリンクは無かった。

表記の 2 点は、wording に 4 文書を点検させた 25 件の指摘のうち、利用者が選んだもの。
残りの指摘は直さない。立てた時点で全角括弧は Player.md 23・Admin.md 40・
Developer.md 108 個。

書式を揃えるだけの変更なので、確認は main が行う。
