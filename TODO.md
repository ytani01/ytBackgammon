...
## TODO-068. 文書の見出しに階層の分かる接頭辞を付ける

|      | main | 担当 |
|------|------|------|
| 見込み | Sonnet 5 / effort medium | main のみ |

- [ ] `README.md`・`docs/Player.md`・`docs/Admin.md`・`docs/Developer.md` の
      見出しに、`#` の数と同じ数の `=` を付ける（`## == 見出し`）
- [ ] コードブロックの中の `#` で始まる行は変えない
- [ ] 文書の中に見出しへのアンカーリンクが無いことを確かめる

見出しの階層が分かりにくいため（利用者の依頼）。対象はこの 4 文書だけに決めた。
見出しは 4 文書で 91 個（h1〜h4）。GitHub のアンカーは見出しの文字から作られるので、
外から見出しへ張ったリンクは切れる。立てた時点では、文書の中にアンカーリンクは無かった。
