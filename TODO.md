...
ytbg の複数のサーバを立ち上げ、すべてのボードを一覧できるサーバ（lobby）を作る。
個々のボードは、今までどおり個々のサーバが制御する。

- [ ] `ytbg` をサブコマンドに分ける。ボード 1 面は `ytbg board 1 -p 5001`、一覧サーバは `ytbg lobby -c ytbg.toml`
- [ ] 設定ファイル（TOML、`tomllib` で読む）に、ボードごとに server_id・ポート・画像ディレクトリ・URL（省略可）を書く
- [ ] lobby は起動時に、設定にあるボードをすべて子プロセスとして起動する。lobby を止めると（Ctrl+C・kill）ボードも止まる
- [ ] 一覧ページに、ボードごとの状態（動作中・停止中）と起動・停止ボタンを置く
- [ ] 一覧ページは、選んだ 1 面を大きく、残りを小さく iframe で並べる
- [ ] iframe の URL は、既定では一覧ページを開いたホスト名に各ボードのポートを付けたもの。設定に URL があればそちらを使う
- [ ] `ytbg-boot.sh`・`ytbg-stop.sh`・`ytbg.html` を消し、`ytbg.sh`・README・docs・CLAUDE.md の説明を書き換える
- [ ] テストを足す（lobby の起動・停止、lobby を止めたらボードも止まること）

2026-09-15 に利用者と決めた。

- **子プロセスの面倒を見る範囲:** ボードは lobby の子プロセスとしてだけ動かす。
  落ちたボードは自動で再起動せず、一覧ページに「停止中」と出す。lobby が
  止まったら全ボードを止める。lobby の外で動いているボードを見つけて扱うことはしない
- **起動・停止の操作:** 個別の起動・停止は一覧ページのボタンで行う。
  コマンドで扱うのは lobby の起動と終了だけ（動いている lobby へ指示を送る
  コマンドは作らない）
- **認証はしない。** ボードと同じく、誰でも押せる
- **ボードの数は設定ファイルで固定する。** Web から足したり消したりはしない
- サブコマンド名の `hub` は、`hub.py` の `ClientHub` と紛らわしいので使わない
- `ytbg.html` の Jitsi の埋め込みは無くなる
- ボードが起動できないとき（ポートが塞がっているなど）は、一覧ページに
  「停止中」と出し、理由を lobby のログに残す
- 分担: 実装は implementer（子プロセスの扱いが込み入るので Opus）。確認は
  verifier（起動・停止と、lobby を止めたらボードも止まることを実際に試す）。
  挙動が変わるので、レビュー（子プロセスの扱いと CLI の分岐）を reviewer に分ける
