...
**残っている項目: TODO-067.** これまでに 66 件を決着させた。
...
- ``ytbg lobby`` を "https://www.ytani.net/ytbg" として見せる
- ``ytbg board 1``を "https://www.ytani.net/ytbg1"  としてみせる
- ``ytbg board 2``を "https://www.ytani.net/ytbg2"  としてみせる
- ytbg を起動するホストは、"rpi5-2"、ポートは何でもよい
- 現在のリバースプロキシの設定ファイルは、"~/tmp/nginx.conf"

上記のようにするために、以下について教えて
- "rpi5-2"上での ytbg起動スクリプト
- "nginx.conf"の修正内容
...
- [ ] 答えを `docs/Admin.md` に「設定例」としてまとめる（ホスト名は例示用の名前にする。認証は付けない）
- [ ] 設定例の `ytbg.toml` と起動のコマンドを手元で試す（verifier）
