...
def other_tasks(bg_server):
    ...
    return [t for t in asyncio.all_tasks()
            ...
            and t is not bg_server._replay_task
            and not t.done()]
...
async def test_running_replay_is_stopped_by_next_request(bg_server, req):
    ...
    hist_mid = len(bg_server._history)
    assert hist_mid > 1
    ...
    assert len(bg_server._history) == hist_mid + 1
...
async def test_only_one_replay_runs(bg_server, req, no_sleep):
    ...
    await asyncio.wait_for(bg_server._replay_task, timeout=5)
    assert other_tasks(bg_server) == []
