...
- 一覧ページは iframe を作り直さない(作り直すと読み込み直しになる)。大きく出す
  ...
  偽から真に変わったとき(最初の読み込みも同じ)と、下の大きく出すボードの切り替えのときに
  入れる。**listen する前に入れると、iframe が接続エラーの画面のまま戻らない**
- 音を出すのは大きいボードだけで、ほかの iframe には `?sound=off` を付ける(全面の音が
  重ならないように)。大きく出すボードを切り替えたら、切り替わった 2 面だけ `src` を
  入れ直して読み込み直す。最初に開いたときは、大きく出すボードを選んでから読み込む。
  iframe はボード自身のポート(別のオリジン)なので、`allow="autoplay"` を付けている
- 大きいボードの倍率は、`lobby.js` の `fit_main()` がウィンドウの幅と高さの両方に
  収まる最大の値を計算し、CSS 変数 `--main-scale` で枠の大きさと `scale()` に渡す。
  iframe そのものは 1030×650px のまま。計算し直すのは、大きく出すボードを選んだとき、
  ウィンドウの `resize` のとき、`/api/boards` を読み直したとき
  (見出しの行の高さが状態の文字で変わるため)
...
このほか、Freeモードでの移動(`put_checker`)と目の変更(`dice`)、名前、
...
Freeモード・名前・得点の操作は積む。クロックの操作は `GameInfo` を書き換えないので
...
- 予測に失敗したら何も送らない
- Freeモードの駒の移動は予測しない(ルール判定を通らないので行き先を確かめられない)
...
Freeモードでの目の変更と、得点の ▲▼(Freeモードに限らない)も、
...
- Roll を押した直後に ▲ や Freeモードのダイスを押すと、予測の表示で
...
| `settings.js` | `Settings`(音、Freeモード、PIP の表示、プレーヤー番号)、Cookie、クエリ文字列、`<body>` の `data-*` |
...
- `Drag` は、掴んだ位置をチェッカー用とキューブ用で別に持つ。Freeモードなら
