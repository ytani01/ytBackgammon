...
- **サーバは手番を強制しない。** `turn` は画面での表示と、操作できるかの
  判定に使うだけ。サーバは送り主を確かめず、認証もプレーヤーの割り当ても無い
...
| 戻り値 | 意味 |
|--------|------|
| `None` | ハンドラが自分で送信済み。`on_json()` はそのまま終わる |
...
| オープニングロール | `dice`（2 通）→ `set_turn`（区切り） |
...
`apply_move()` の中では、次の順番を守る必要がある。使ったダイスの組み合わせは
