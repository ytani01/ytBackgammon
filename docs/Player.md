...
複数のボードをまとめて立てている場合は、一覧ページ(ポートは既定で 5000)で
全部のボードを見られる。「大きく表示」で選んだボードが大きく出る。
音が出るのは大きく出ているボードだけ。ほかのボードの音を聞きたいときは、
「大きく表示」で切り替えるか、ボード名を押して別のタブで開く。
...
| 4 | PIP |
| 5 | スコア |
| 6 | 投了ボタン。その下に ボード回転・1 つ進める・1 つ戻す が縦に並ぶ |
...
スコアは普通は自動で付く。直したいときは、**スコアの数字のあたりを押すと
1 点ずつ増減する**(左が ＋1、右が −1。ボタンの見た目は出ていない)。
