...
実際のボードを囲んでいるのに近い。ビデオ通話や音声通話をつなぎ、
話しながら遊ぶのがおすすめ。
...
|------|------------|
| 三本線のアイコン | メニューを開く |
| Sound | 音のオン・オフ（**自分の画面だけ**） |
| Free | ルールチェックを外す（**全員に効く**） |
| Pip | PIP（残りの進む数）の表示 |
| Clock | 持ち時間のオン・オフ |
| `m +` と `s` の数値 | 持ち時間（分）と 1 手ごとの猶予（秒） |

![ヘッダ](images/header.png)

左から、1 メニュー、2 Sound、3 Free、4 Pip、5 Clock、6 持ち時間の設定。

### 盤面

- **チェッカー**（駒）、**ダイス**、**キューブ**（ダブリングキューブ）
- **ダイスカップ** — 押すとダイスを振る
- プレーヤーごとの**名前の入力欄**、**スコア**、**持ち時間**
- 盤面の外の 4 つのボタン — **投了**、**ボード回転**、**1 つ進める**、**1 つ戻す**
- 場面に応じて出るバナー — **パス**、**勝ち**、**投了**
...
最初（オープニング）は両方が 1 個ずつ振り、大きい目が出た側が先手になる。
同じ目なら振り直し。
