...
`url` は `http://` か `https://` で始まる URL で書く。`/board1/` のような
パスだけの URL は書けない（ボードは WebSocket を `/ws` でつなぐので、
パスで分けたリバースプロキシの裏では動かない）。
...
- **iframe の URL** は、設定に `url` があればそれ、無ければ一覧ページを開いた
  ホスト名にボードのポートを付けたもの。リバースプロキシの裏でボードを
  別の名前で出しているときは `url` を書く
