...
- **サーバは、今の盤面と合わない操作を捨てる。** 何をするかを送るので、
  2 人がほぼ同時に押すと 2 回ぶん効いてしまう（キューブが 2 ではなく 4 に
  なる、得点が 2 回足される）ため。防ぐのは重複だけで、ルールの判定は
  クライアントのまま。捨てるのは次のとき。捨てたときは履歴にも積まず、
  盤面も送らない
  - `double`: 「テイク済みで、中央か自分の側にあるキューブ」でも
    「自分に掛けられたキューブ（リダブル）」でもない
  - `take`: テイク済みか、キューブが自分の側に無い
  - `cancel_double`: テイク済みか、キューブが相手の側に無い
  - `resign`: 勝負のついたあと（`turn` が -1）
  - `end_turn`: 手番でないプレーヤーから
  - `opening`: `turn` が 2 未満（どちらかの手番か、勝負のついたあと）
  - `move` は捨てずに、勝負のついたあとなら得点を足さない
- サーバは届いた `data` の値の型（数、真偽値、配列とその中身）を入口で確かめ、
  合わなければ盤面を書き換える前に例外にする。クライアントも数に直して送る
...
**「戻る」「進む」（履歴の操作）で勝負のついた盤面に来ても、クロックは止めない。**
止めたいときは、クロックを押して止める。
