...
| type | 載せるもの | サーバが行うこと |
|------|------------|------------------|
| `roll` | `player`, `dice` | そのプレーヤーのダイスを入れる。使えない目は 11〜16 のまま |
| `opening` | `winner`（同じ目なら -1） | 勝った側のダイスを `[勝者の目, 0, 0, 敗者の目]` にし、負けた側のダイスを空にして、手番を勝った側に渡す。同じ目なら両方を空にし、`turn` を 2 に戻す |
| `move` | `player`, `moves`（`[{ch, p, idx}]`）, `dice`, `score` | 駒を置き、ダイスを入れる。`score` が 1 以上なら `turn` を -1 にして得点を足す（上限 99） |
...
| `cancel_double` | `player`（掛けた側） | キューブの値を半分に戻し、掛けた側に置く（1 に戻ったら中央）。クロックを切り替える |
| `resign` | `player`, `score` | `turn` を -1 に、`resign` を `player` にし、相手に得点を足す（上限 99） |

- パスのバナーを押したときも `end_turn` を送る
- オープニングで先手が決まっても、クロックは動かさない（今と同じ）
- クロックの切り替えでは、止める側のクロックを止めてから、もう片方を
  猶予を戻して動かす。止める側は、`double` と `take` では `player`、
  `cancel_double` ではその相手になる

### 勝負がついたときのクロック

**`turn` が -1 になったら、サーバが両方のクロックを止める。** 勝敗の判定は
これまでどおりクライアントが行い、その結果を `move` や `resign` に載せる。
...
- `set_clock_limit` と `set_clock_switch` を受けたら、サーバが両方の
  クロックを止める。今はクライアントが別に `stop_clock` を 2 通送っている
...
- `Board.set_turn()` はサーバへ送らない。そのために付いていた引数も消す
- `Board.winner_is()` は判定するだけで、状態を書き換えない
- 音とダイスの回転は、今と同じく `last_op`（直前の操作）から出す。
...
「使ったダイスを暗くするのは `apply()` のあと」という順番の縛りがなくなる。

### 判定は `gameinfo` だけを読む

部品が持っている状態の写しをなくし、判定は `board.gameinfo` を読む。
...
- `Board` に残るのは、部品を作ること、`apply()`、盤面の反転、ルール層を
  呼ぶ薄い関数
...
1. **サーバに名前付きの操作を足し、登録表を 1 つにする。** 古い type は
   まだ残し、履歴に積むかどうかもメッセージの `history` を見たままにする。
   今のクライアントのまま動き、テストも通る状態を保つ
2. **クライアントを `actions.js` から 1 通で送る形に切り替える。**
   使わなくなった type と、メッセージの `history` をここで消す
3. **判定が `gameinfo` を読むようにする**
4. **`Board` からドラッグと設定を切り出す**
5. **部品に id ではなく要素を渡す**
6. **サーバの細かい修正**（`server.py` を 1 と取り合うので最後にする）
