...
class Checker extends PlayerItem {
     ...
     *                    0: そこには移動できない
     */
    dice_check(player, active_dice, from_p, to_p) {
        console.log(`dice_check(player=${player}, `
        ...
        console.log(`dice_chek> ${from_p} ==> ${to_p}`);
    ...
    on_mouse_up(e) {
        ...
        const dice_value = this.dice_check(this.player, active_dice,
                                           ch.cur_point, dst_p);
        if ( dice_value == 0 ) {
            this.cancel_move(ch);
            return;
